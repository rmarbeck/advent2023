...
object Solution:
  def run(inputLines: Seq[String]): (String, String) =
    ...
    val graph =  GraphFromArray(summits)(next)
    val start = summits.find:
      case Summit(0, 0, _, _, 0) => true
      case _ => false
    .get

    val end = summits.toList.filter:
      case Summit(row, col, _, _, _) if row == height - 1 && col == width - 1 => true
      case _ => false

    val resultPart1 = Dijkstra.solve(graph, start, end).drop(1).map(_.value).sum
  ...
  def next(summit: Summit)(using heatMap: HeatMap): List[Summit] =
